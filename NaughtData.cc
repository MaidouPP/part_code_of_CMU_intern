...
NaughtData::extractNaughts(const Sketch<bool> &sketch,
  ...
  const int THRESHOLD = 120; // for detection

  for ( Region reg : regionlist ) {
    Point center = reg.findCentroid();
    bool good = true;
    float semimaj = 0, semimin = 4000;
    float theta = 0.0;
	...
	if (semimaj < (float)(r1+r-1)/2) {
      ...
      float theta0 = theta;
      float semimaj0 = semimaj;
      float semimin0 = semimin;
      // adjust the semimajor and semiminor
      for( int i = -4; i<=4 ; i++ ) {
        for( int j = -4; j<=4; j++ ) {
          for( int k = -5; k<=5; k++ ) {
            float semimaj1 = semimaj0 + i;
            float semimin1 = semimin0 + j;
